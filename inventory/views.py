...
from .models import Asset, AuditLog, LaptopSpec, MonitorSpec, CellphoneSpec, OtherSpec
from .forms import (
    AssetForm, LaptopSpecForm, MonitorSpecForm, CellphoneSpecForm, 
    OtherSpecForm, CustomUserCreationForm
)
...
def asset_list(request):
        ...
        if request.user.is_superuser or request.user.role in ['ADMIN', 'IT']:
            ...
            if asset_type == 'LAPTOP':
                LaptopSpec.objects.create(
                    ...
                    ram=request.POST.get('ram'), cpu=request.POST.get('cpu'), gpu=request.POST.get('gpu')
                )
            elif asset_type == 'MONITOR':
                MonitorSpec.objects.create(asset=asset, brand=request.POST.get('brand'), model=request.POST.get('model'))
            elif asset_type == 'CELLPHONE':
                CellphoneSpec.objects.create(asset=asset, brand=request.POST.get('brand'), model=request.POST.get('model'))
    ...
    assets = Asset.objects.all().select_related('created_by', 'updated_by').prefetch_related('audit_logs__auditor').order_by('-date_added')
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        for asset in assets:
            ...
            local_updated = timezone.localtime(asset.date_updated).strftime('%Y-%m-%d %H:%M') if asset.date_updated else ''
            ...
            latest_auditor = (latest_log.auditor.get_full_name() or latest_log.auditor.username) if latest_log and latest_log.auditor else ''
            latest_audit_date = timezone.localtime(latest_log.audit_date).strftime('%Y-%m-%d %H:%M') if latest_log else ''
...
def audit_scan(request):
    ...
    return render(request, 'inventory/audit_scan.html', {
        'asset': asset,
...
def export_csv(request):
    ...
    writer.writerow([
        'Company Tag', 'Unit Name', 'Serial Number', 'Status', 
        'Assigned To', 'Created By', 'Date Added', 'Updated By', 
        'Date Updated', 'Latest Audit Remarks'
    ])
    ...
    for asset in assets:
        ...
        local_date_updated = timezone.localtime(asset.date_updated).strftime('%Y-%m-%d %H:%M') if asset.date_updated else ''
...
def export_pdf(request):
    ...
    title_style = ParagraphStyle('DocTitle', parent=styles['Heading1'], fontSize=18, leading=22, textColor=colors.HexColor('#2C3E50'), fontName='Helvetica-Bold')
    subtitle_style = ParagraphStyle('DocSubTitle', parent=styles['Normal'], fontSize=9, leading=11, textColor=colors.HexColor('#7F8C8D'))
    cell_style = ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, leading=10, textColor=colors.HexColor('#2C3E50'))
    header_cell_style = ParagraphStyle('TableHeaderCell', parent=styles['Normal'], fontSize=9, leading=11, textColor=colors.white, fontName='Helvetica-Bold')
    ...
    elements.append(Paragraph(f"Generated on: {current_time} | Auditor: {request.user.get_full_name() or request.user.username}", subtitle_style))
    ...
    sig_data = [
        [Paragraph("<b>Audited By:</b> " + (request.user.get_full_name() or request.user.username), sig_cell_style), Paragraph("<b>Assisted By:</b> ___________________________", sig_cell_style)],
        [Paragraph("<b>Signature:</b> ___________________________", sig_cell_style), Paragraph("<b>Signature:</b> ___________________________", sig_cell_style)],
        [Paragraph("<b>Date:</b> " + current_time.split(' ')[0], sig_cell_style), Paragraph("<b>Date:</b> ___________________________", sig_cell_style)]
    ]
...
def generate_accountability_pdf(request, pk):
    ...
    terms = "By signing below, the employee acknowledges receipt of the asset listed above in good working condition and agrees to follow company asset security and usage policies."
    elements.append(Paragraph(terms, body_style))
    elements.append(Spacer(1, 0.6 * inch))

    # Signature Block Table
    sig_data = [
        [Paragraph(f"<b>Issued By (IT Dept):</b> {request.user.get_full_name() or request.user.username}", body_style),
